    ...
    F: Fn(Option<(String, String)>) + 'static,
{
    ...
    let default_desc = trf(
        "System recovery point {0}",
    ...
    // Description entry
    let desc_label = Label::new(Some(&tr("Description:")));
    desc_label.set_halign(gtk::Align::Start);
    content.append(&desc_label);

    let desc_entry = Entry::new();
    desc_entry.set_text(&default_desc);
    desc_entry.set_placeholder_text(Some(&tr("e.g., Before Docker installation")));
    desc_entry.set_activates_default(true);
    content.append(&desc_entry);

    // Info label
    let info = Label::new(Some(&tr(
        "The recovery point will be named automatically from the current date and time.",
    )));
    info.set_wrap(true);
