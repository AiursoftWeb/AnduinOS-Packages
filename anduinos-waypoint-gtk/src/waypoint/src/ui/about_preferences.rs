...
pub fn show_about_dialog(window: &adw::ApplicationWindow) {
    ...
    dialog.set_title(Some(&tr("About AnduinOS Waypoint")));
    ...
    // Application name
    let name_label = Label::new(Some(&tr("AnduinOS Waypoint")));
    name_label.add_css_class("title-1");
    content.append(&name_label);

    // Version
    let version_label = Label::new(Some(&trf("Version {0}", &[env!("CARGO_PKG_VERSION")])));
    version_label.add_css_class("dim-label");
    content.append(&version_label);

    // Description
    let description = Label::new(Some(&tr(
        "System recovery, scheduling, retention, and external backup management for AnduinOS.",
    )));
    ...
    // GitHub link
    let github_btn = gtk::Button::with_label(&tr("View on GitHub"));
    github_btn.add_css_class("flat");
    github_btn.connect_clicked(|_| {
        let _ = std::process::Command::new("xdg-open")
            .arg("https://github.com/AiursoftWeb/AnduinOS-Packages/tree/master/anduinos-waypoint-gtk")
            .spawn();
    });
    links_box.append(&github_btn);

    // Report issue link
    let issue_btn = gtk::Button::with_label(&tr("Report an issue"));
    issue_btn.add_css_class("flat");
    issue_btn.connect_clicked(|_| {
        let _ = std::process::Command::new("xdg-open")
            .arg("https://github.com/AiursoftWeb/AnduinOS-Packages/issues")
            .spawn();
    });
