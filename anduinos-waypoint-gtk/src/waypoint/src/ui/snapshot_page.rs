...
use crate::dbus_client::{RecoveryEngineStatus, VerificationResult, WaypointHelperClient};
...
mod imp {
    ...
    impl ObjectSubclass for SnapshotPage {
        const NAME: &'static str = "WaypointSnapshotPage";
...
impl SnapshotPage {
    ...
    fn setup_ui(&self) {
        ...
        let unsupported = status_page(
            ...
            Some(&tr("Waypoint requires the standard AnduinOS Btrfs layout.")),
        );
    ...
    pub fn refresh(&self) {
        ...
        glib::spawn_future_local(async move {
            let loaded = gio::spawn_blocking(|| {
                WaypointHelperClient::new().and_then(|client| client.recovery_engine_status())
            })
            .await
            ...
            let description = status
                ...
                .unwrap_or_else(|| {
                    if let Some(filesystem) = status.layout.root_filesystem.as_deref() {
                        ...
                        tr("Waypoint requires the standard AnduinOS Btrfs layout.")
                    }
                });
    ...
    fn verify_then_confirm_rollback(&self, item: SnapshotItem) {
        ...
        run_operation(
            ...
            move || WaypointHelperClient::new()?.verify_snapshot(id),
            move |parent, result| match result {
    ...
    fn schedule_rollback(&self, id: String) {
        ...
        run_operation(
            ...
            move || {
                let result = WaypointHelperClient::new()?.schedule_deployment_restore(id)?;
                if !result.0 {
                    anyhow::bail!(result.1);
                }
                Ok(())
    ...
    fn cancel_pending_rollback(&self) {
        ...
        run_operation(
            ...
            move || {
                let result = WaypointHelperClient::new()?.cancel_deployment_restore()?;
                if !result.0 {
                    anyhow::bail!(result.1);
                }
                Ok(())
