...
use super::snapshot_row::{SnapshotAction, SnapshotRow};
...
/// * `action_handler` - Callback to handle snapshot actions (delete, restore, verify, etc.)
///
/// # Behavior
...
pub fn refresh_snapshot_list_internal(
    ...
    let all_snapshots = match manager.borrow().load_snapshots() {
        Ok(s) => s,
        Err(e) => {
            let unsupported_layout = e.is_unsupported_layout();
            if unsupported_layout {
                log::info!("Recovery points are unavailable on this filesystem layout: {e}");
            } else {
                log::warn!("Could not load recovery points: {e}");
            }

            compare_btn.set_sensitive(false);
            compare_btn.set_tooltip_text(Some(&tr("Recovery points are unavailable")));
            if let Some(button) = create_btn {
                button.set_sensitive(false);
            }
            if let Some(label) = match_label {
                label.set_text(&tr("Recovery unavailable"));
            }

            let placeholder = adw::StatusPage::new();
            placeholder.set_title(&tr("Recovery Points Unavailable"));
            if unsupported_layout {
                placeholder.set_description(Some(&tr(
                    "This system does not use the supported AnduinOS Btrfs layout.",
                )));
                placeholder.set_icon_name(Some("drive-harddisk-symbolic"));
            } else {
                placeholder.set_description(Some(&tr(
                    "The recovery service could not load recovery points. Try reopening the app.",
                )));
                placeholder.set_icon_name(Some("dialog-warning-symbolic"));
            }
    ...
    if let Some(label) = match_label {
        ...
        if is_filtered {
            label.set_text(&trf(
                "Showing {0} of {1} recovery points",
                &[
                    &filtered_snapshots.len().to_string(),
                    &all_snapshots.len().to_string(),
                ],
            ));
        } else {
            label.set_text(&trf(
    ...
    if filtered_snapshots.len() < 2 {
        compare_btn.set_sensitive(false);
        compare_btn.set_tooltip_text(Some(&tr(
            "At least 2 recovery points are needed to compare",
        )));
    } else {
        compare_btn.set_sensitive(true);
        compare_btn.set_tooltip_text(Some(&tr("Compare packages between recovery points")));
    }
    ...
    if filtered_snapshots.is_empty() {
        ...
        if all_snapshots.is_empty() {
            placeholder.set_title(&tr("No Recovery Points Yet"));
            placeholder.set_description(Some(&tr(
                "Recovery points let you roll back your system to a previous state",
            )));
            placeholder.set_icon_name(Some("org.anduinos.Waypoint"));

            // Add prominent "Create Restore Point" button if create_btn is provided
            if let Some(main_create_btn) = create_btn {
                let create_button =
                    gtk::Button::with_label(&tr("Create Your First Recovery Point"));
            ...
            placeholder.set_title(&tr("No Matching Recovery Points"));
            placeholder.set_description(Some(&tr(
                "No recovery points match your search criteria.\n\nTry adjusting your search or filter settings.",
            )));
        ...
        // The trusted deployment record owns protection state. User preferences
        // remain local-only notes and never pretend to protect a recovery point.
        let (pinned, regular): (Vec<_>, Vec<_>) = filtered_snapshots
            .into_iter()
            .partition(|snapshot| snapshot.pinned);

        // Add pinned snapshots section if any exist
        if !pinned.is_empty() {
            // Add section header for pinned snapshots
            let pinned_header = adw::ActionRow::new();
            pinned_header.set_title(&tr("Pinned Recovery Points"));
            ...
            if !regular.is_empty() {
                ...
                regular_header.set_title(&tr("All Recovery Points"));
        ...
        // Recovery point rows contain only bounded deployment metadata.
