...
impl ScheduleCard {
    ...
    pub fn new(schedule: Schedule) -> Self {
        ...
        // Next run
        let next_run_label = Label::new(Some(&tr("Next run: calculating...")));
        next_run_label.add_css_class("body");
        next_run_label.set_halign(gtk::Align::Start);
        info_box.append(&next_run_label);

        // Last run
        let last_run_label = Label::new(Some(&tr("Last success: never")));
        ...
        let retention_text = Self::retention_summary(&schedule, prefix_display);
    ...
    fn retention_summary(schedule: &Schedule, prefix: &str) -> String {
        if schedule.timeline_retention.is_some() {
            trf("Retention: timeline policy • Prefix: {0}-", &[prefix])
        } else {
            trf("Retention: legacy policy • Prefix: {0}-", &[prefix])
        }
    }
    ...
    pub fn set_next_run(&mut self, text: &str) {
        self.next_run_label.set_text(&trf("Next run: {0}", &[text]));
    }
    ...
    pub fn set_last_run(&mut self, text: &str, success: bool) {
        ...
        self.last_run_label
            .set_text(&trf("Last success: {0} {1}", &[text, icon]));
    }
