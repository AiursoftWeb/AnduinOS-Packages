...
pub const APP_ID: &str = "org.anduinos.Waypoint";

mod imp {
    use super::*;

    #[derive(Default)]
    pub struct WaypointApplication {
    ...
    impl ObjectSubclass for WaypointApplication {
        const NAME: &'static str = "WaypointApplication";
        type Type = super::WaypointApplication;
        type ParentType = adw::Application;
    }

    impl ObjectImpl for WaypointApplication {}

    impl ApplicationImpl for WaypointApplication {
        fn startup(&self) {
            self.parent_startup();
            let app = self.obj();
            app.load_css();
    ...
    impl GtkApplicationImpl for WaypointApplication {}
    impl AdwApplicationImpl for WaypointApplication {}
}

glib::wrapper! {
    pub struct WaypointApplication(ObjectSubclass<imp::WaypointApplication>)
...
impl WaypointApplication {
    ...
    fn show_about(&self) {
        let about = adw::AboutWindow::builder()
            .application_name(tr("AnduinOS Waypoint"))
            .application_icon(APP_ID)
            .developer_name(tr("AnduinOS Team"))
    ...
    fn load_css(&self) {
        ...
        if let Some(display) = gtk::gdk::Display::default() {
            ...
            log::warn!("Waypoint started without a graphical display");
        }
    }

    fn schedule_smoke_exit(&self) {
        if std::env::var_os("ANDUINOS_WAYPOINT_UI_SMOKE_TEST").is_none()
...
mod tests {
    ...
    fn application_identity_is_stable() {
        assert_eq!(APP_ID, "org.anduinos.Waypoint");
    }
}
