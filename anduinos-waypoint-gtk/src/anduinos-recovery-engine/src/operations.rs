...
impl OperationEngine<SystemCommandRunner> {
    /// Receive one previously verified full Btrfs send stream and register it
    /// as a fresh local recovery point.
    ///
        ...
        F: FnMut(OperationPhase, f64, &str),
    {
        progress(
            OperationPhase::Validate,
            0.02,
            "Validating imported recovery data",
        );
        ensure_supported_layout(layout)?;
        source.validate().map_err(|error| {
            OperationError::new(
                OperationErrorCode::InvalidIdentity,
                format!("Imported recovery metadata is invalid: {error}"),
            )
        })?;
        if !source.can_restore() {
            return Err(OperationError::new(
                OperationErrorCode::InvalidIdentity,
                "Imported recovery metadata is not restorable",
            ));
        }
        self.ensure_transaction_reserve()?;
        self.ensure_store_directories()?;
        ...
        if !deployments.issues.is_empty() {
            return Err(OperationError::new(
                OperationErrorCode::InvalidIdentity,
                "Imported recovery cannot be registered while local metadata has unresolved issues",
            ));
        }
        let lineage_store = LineageStore::new(&self.snapshot_root);
        let _lineage = lineage_store
            .ensure_initialized(&deployments.deployments)
            .map_err(lineage_error)?;
        ...
        let mut record = DeploymentRecord {
            ...
            id,
            parent_id: None,
            kind: DeploymentKind::Imported,
            state: DeploymentState::Creating,
            created_at: Utc::now(),
            title: source.title.clone(),
            reason: format!(
                "Imported from an external backup created {} — {}",
            ...
            snapshot_uuid: None,
            snapshot_parent_uuid: None,
            kernel_release: source.kernel_release.clone(),
            initramfs_sha256: source.initramfs_sha256.clone(),
            boot_artifact_sha256: source.boot_artifact_sha256.clone(),
            dpkg_status_sha256: source.dpkg_status_sha256.clone(),
            mok_certificate_sha256: source.mok_certificate_sha256.clone(),
            pinned: false,
            failure: None,
        };
        if record.reason.chars().count() > 500 {
        ...
        let import_root = self.snapshot_root.join("import-staging");
        ensure_directory(&import_root, 0o700)?;
        let staging = import_root.join(format!("{}.{}", id, Uuid::new_v4().hyphenated()));
        ensure_new_directory(&staging, 0o700)?;
        let received = staging.join("root");
        let deployment_dir = self.deployment_dir(id);
        let snapshot = deployment_dir.join("root");
        ...
        let result: Result<(), OperationError> = (|| {
            progress(
                OperationPhase::Snapshot,
                0.12,
                "Inspecting the Btrfs send stream",
            );
            run_btrfs_receive_dump(stream)?;
            progress(
                OperationPhase::Snapshot,
                0.25,
                "Receiving the external recovery point",
            );
            run_btrfs_receive(stream, &staging)?;
            ensure_only_received_root(&staging)?;
            self.verify_read_only(&received)?;

            progress(
                OperationPhase::CaptureIdentity,
                0.62,
                "Verifying imported system identity",
            );
            verify_imported_content(&received, source)?;
            let (snapshot_uuid, parent_uuid) = self.snapshot_identity(&received)?;

            ensure_new_directory(&deployment_dir, 0o700)?;
            fs::rename(&received, &snapshot).map_err(|error| {
                io_error(
                    "Could not move imported recovery point into the local store",
                    error,
                )
            })?;
            fs::remove_dir(&staging)
                .map_err(|error| io_error("Could not remove import staging directory", error))?;
            sync_directory(&import_root)
                .map_err(|error| io_error("Could not sync the import staging directory", error))?;
            ...
            progress(
                OperationPhase::Commit,
                0.92,
                "Registering imported recovery metadata",
            );
            self.write_record_atomic(&record)?;
            self.sync_btrfs()?;
            lineage_store
                .record_detached_recovery_point(&record)
                .map_err(lineage_error)?;
            Ok(())
        })();

        if let Err(error) = result {
            let _ = self.cleanup_snapshot(&snapshot, &deployment_dir);
            let _ = cleanup_import_staging(&received, &staging, self);
            record.state = DeploymentState::Incomplete;
            record.snapshot_uuid = None;
            record.snapshot_parent_uuid = None;
            record.failure = Some(truncate_failure(error.to_string()));
            let _ = self.write_record_atomic(&record);
            return Err(error);
        }
        progress(
            OperationPhase::Commit,
            1.0,
            "External recovery point imported",
        );
        Ok(record)
    }
}
...
fn run_btrfs_receive_dump(stream: &mut File) -> Result<(), OperationError> {
    stream
        ...
        .map_err(|error| io_error("Could not rewind external backup stream", error))?;
    let input = stream
        .try_clone()
        .map_err(|error| io_error("Could not duplicate external backup stream", error))?;
    let output = Command::new(BTRFS)
        ...
        .map_err(|error| {
            OperationError::new(
                OperationErrorCode::CommandFailed,
                format!("Could not inspect Btrfs send stream: {error}"),
            )
        })?;
    if !output.status.success() {
        return Err(OperationError::new(
            OperationErrorCode::InvalidIdentity,
            format!(
                "Btrfs send stream is invalid{}",
                command_diagnostic_suffix(&output.stderr)
            ),
        ));
    }
    Ok(())
...
fn run_btrfs_receive(stream: &mut File, staging: &Path) -> Result<(), OperationError> {
    ...
    let output = Command::new(BTRFS)
        ...
        .map_err(|error| {
            OperationError::new(
                OperationErrorCode::CommandFailed,
                format!("Could not receive external recovery point: {error}"),
            )
        })?;
    if !output.status.success() {
        return Err(OperationError::new(
            OperationErrorCode::CommandFailed,
            format!(
                "Could not receive external recovery point{}",
                command_diagnostic_suffix(&output.stderr)
            ),
        ));
    }
...
fn ensure_only_received_root(staging: &Path) -> Result<(), OperationError> {
    let entries = fs::read_dir(staging)
        ...
        .map_err(|error| io_error("Could not inspect received recovery point", error))?;
    if entries.len() != 1 || entries[0].file_name() != "root" {
        return Err(OperationError::new(
            OperationErrorCode::InvalidIdentity,
            "External backup must contain exactly one System recovery subvolume",
        ));
    }
    let metadata = fs::symlink_metadata(entries[0].path())
        .map_err(|error| io_error("Could not inspect received recovery root", error))?;
    if !metadata.file_type().is_dir() {
        return Err(OperationError::new(
            OperationErrorCode::UnsafePath,
            "Received recovery root is not a real directory",
        ));
    }
    Ok(())
}
...
fn verify_imported_content(
    ...
    let kernel = source.kernel_release.as_deref().ok_or_else(|| {
        OperationError::new(
            OperationErrorCode::InvalidIdentity,
            "Imported recovery metadata has no kernel release",
        )
    })?;
    verify_digest(
        "imported initramfs",
        source.initramfs_sha256.as_deref(),
        &hash_regular_file(&received.join("boot").join(format!("initrd.img-{kernel}")))?,
    )?;
    verify_digest(
        "imported kernel boot artifact",
        source.boot_artifact_sha256.as_deref(),
        &hash_regular_file(&received.join("boot").join(format!("vmlinuz-{kernel}")))?,
    )?;
    verify_digest(
        "imported dpkg database",
        source.dpkg_status_sha256.as_deref(),
        &hash_regular_file(&received.join("var/lib/dpkg/status"))?,
    )?;
    if hash_optional_regular_file(&received.join(MOK_CERTIFICATE))? != source.mok_certificate_sha256
    {
        return Err(identity_mismatch("imported MOK certificate"));
    }
    Ok(())
}
...
fn cleanup_import_staging(
    received: &Path,
    staging: &Path,
    engine: &OperationEngine<SystemCommandRunner>,
) -> Result<(), OperationError> {
    let _ = received;
    let entries = match fs::read_dir(staging) {
        Ok(entries) => entries
            .collect::<Result<Vec<_>, _>>()
            .map_err(|error| io_error("Could not inspect import staging cleanup", error))?,
        Err(error) if error.kind() == io::ErrorKind::NotFound => return Ok(()),
        Err(error) => return Err(io_error("Could not inspect import staging cleanup", error)),
    };
    for entry in entries {
        let path = entry.path();
        let metadata = fs::symlink_metadata(&path)
            .map_err(|error| io_error("Could not inspect an import staging entry", error))?;
        if metadata.file_type().is_dir() {
            if engine
                .run_btrfs(&[
                    OsString::from("subvolume"),
                ...
                .is_err()
            {
                fs::remove_dir_all(&path).map_err(|error| {
                    io_error("Could not remove an import staging directory", error)
                })?;
            }
        } else {
            fs::remove_file(&path)
                .map_err(|error| io_error("Could not remove an import staging entry", error))?;
        }
    }
    match fs::remove_dir(staging) {
        Ok(()) => Ok(()),
        Err(error) if error.kind() == io::ErrorKind::NotFound => Ok(()),
        Err(error) => Err(io_error("Could not remove import staging directory", error)),
    }
}
...
mod tests {
    ...
    impl TestEnvironment {
        fn new() -> Self {
            let base = std::env::temp_dir().join(format!("waypoint-operation-{}", Uuid::new_v4()));
            let system_root = base.join("system");
            let snapshot_parent = base.join("snapshots");
            let snapshot_root = snapshot_parent.join("anduinos");
            for relative in [
                "boot/initrd.img-test-kernel",
                "boot/vmlinuz-test-kernel",
                "var/lib/dpkg/status",
