...
impl RollbackTransaction {
    pub fn new(
        ...
        Self {
            ...
            grub_entry_id: format!("anduinos-waypoint-{id}"),
            failure: None,
        }
    }
    ...
    pub fn old_root_name(&self) -> String {
        format!("@root.waypoint-old-{}", self.id)
    }

    pub fn new_root_name(&self) -> String {
        format!("@root.waypoint-new-{}", self.id)
    }
    ...
    pub fn validate(&self) -> Result<(), TransactionError> {
        ...
        if self.grub_entry_id != format!("anduinos-waypoint-{}", self.id) {
            return Err(TransactionError::new(
                TransactionErrorCode::InvalidRecord,
...
mod tests {
    ...
    fn transaction_names_are_deterministic_and_path_safe() {
        ...
        assert_eq!(
            ...
            format!("anduinos-waypoint-{}", transaction.id)
        );
        assert_eq!(
            transaction.old_root_name(),
            format!("@root.waypoint-old-{}", transaction.id)
        );
        assert_eq!(
            transaction.new_root_name(),
            format!("@root.waypoint-new-{}", transaction.id)
        );
    }
