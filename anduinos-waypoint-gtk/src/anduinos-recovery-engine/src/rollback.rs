...
const COMMAND_PATH: &str =
    "/usr/libexec/anduinos-waypoint/no-os-prober:/usr/sbin:/usr/bin:/sbin:/bin";
...
fn verify_grub_config(path: &Path, transaction: &RollbackTransaction) -> Result<(), RollbackError> {
    ...
    let request_marker = format!("anduinos.waypoint={}", transaction.id);
...
mod tests {
    ...
    fn grub_config_requires_exactly_one_transaction_entry() {
        ...
        transaction
            .transition(RollbackPhase::Armed, Utc::now())
            .unwrap();
        let root = std::env::temp_dir().join(format!("waypoint-grub-{}", uuid::Uuid::new_v4()));
        let line = format!(
            "menuentry x --id '{}' {{ linux anduinos.waypoint={} }}\n",
