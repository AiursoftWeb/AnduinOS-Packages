...
pub const GRUB_EXTERNAL_ENVIRONMENT: &str = "/boot/efi/EFI/anduinos/waypoint-grubenv";
pub const GRUB_NEXT_ENTRY_VARIABLE: &str = "waypoint_next_entry";
...
impl<R: BootToolRunner> BootIntegration<R> {
    ...
    pub fn recovery_menu_entry(&self) -> Result<Option<String>, BootError> {
        ...
        Ok(Some(format!(
            "menuentry 'AnduinOS Waypoint recovery' --class anduinos --class gnu-linux --id '{entry_id}' {{\n\
             \tinsmod btrfs\n\
             \tsearch --no-floppy --fs-uuid --set=root {fs_uuid}\n\
             \techo 'Starting AnduinOS system recovery…'\n\
             \tlinux {kernel_path} root=UUID={fs_uuid} ro rootflags=subvol=@root anduinos.waypoint={id}\n\
    ...
    /// Clear Waypoint's selector without treating an already-clear GRUB
...
mod tests {
    ...
    impl Environment {
        fn new() -> Self {
            let root = std::env::temp_dir().join(format!("waypoint-boot-{}", uuid::Uuid::new_v4()));
    ...
    fn external_environment_file_has_exact_shape() {
        let root =
            std::env::temp_dir().join(format!("waypoint-grubenv-shape-{}", uuid::Uuid::new_v4()));
    ...
    fn emits_transaction_bound_grub_entry() {
        ...
        assert!(entry.contains(&format!("anduinos.waypoint={}", environment.transaction.id)));
