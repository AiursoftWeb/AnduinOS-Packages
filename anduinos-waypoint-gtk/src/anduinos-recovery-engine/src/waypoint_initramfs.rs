...
fn main() -> ExitCode {
    ...
    let requested = match arguments.as_slice() {
        ...
        _ => {
            eprintln!("Usage: anduinos-waypoint-initramfs [ROLLBACK_ID]");
            return ExitCode::from(64);
        }
    };
    ...
    match RecoveryEngine::default().execute_with_observer(requested, &boot_id, print_checkpoint) {
        ...
        Ok(RecoveryOutcome::Applied) => {
            eprintln!("AnduinOS Waypoint activated the selected recovery point");
            ExitCode::SUCCESS
        }
        Ok(RecoveryOutcome::Reverted) => {
            eprintln!("AnduinOS Waypoint restored the protected fallback root");
            ExitCode::SUCCESS
        }
        Err(error) => {
            eprintln!("AnduinOS Waypoint recovery failed: {error}");
            ExitCode::FAILURE
        }
    }
}
...
fn print_checkpoint(checkpoint: RecoveryCheckpoint) {
    eprintln!("WAYPOINT-CHECKPOINT {}", checkpoint.as_str());
}
