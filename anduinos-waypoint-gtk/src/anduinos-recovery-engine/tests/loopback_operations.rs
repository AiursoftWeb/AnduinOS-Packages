...
fn fixture_root() -> PathBuf {
    let root = PathBuf::from(
        env::var_os("ANDUINOS_WAYPOINT_LOOPBACK_ROOT")
    ...
    assert!(
        test_directory.parent() == Some(Path::new("/tmp"))
            && test_directory_name.starts_with("anduinos-waypoint-operations.")
...
fn supported_layout() -> LayoutReport {
    ...
    .map(|(mount_point, subvolume)| MountReport {
        ...
        source: "/dev/loop-waypoint-test".into(),
    })
    .collect();
    LayoutReport {
        support: LayoutSupport::Supported,
        root_filesystem: Some("btrfs".into()),
        root_source: Some("/dev/loop-waypoint-test".into()),
        issues: Vec::new(),
        mounts,
    }
}
...
fn real_btrfs_create_verify_protect_retention_and_cleanup() {
    ...
    let store_root = root.join("@snapshots/anduinos-waypoint");
...
fn real_btrfs_rejects_malformed_layouts_without_mutation() {
    ...
    let root = fixture_root();
    let system_root = root.join("@root");
    let store_root = root.join("@snapshots/anduinos-waypoint-malformed");
    let engine = OperationEngine::new(&system_root, &store_root, SystemCommandRunner);

    let full_store_root = root.join("@snapshots/anduinos-waypoint-no-space");
    ...
    let rejected = [
        ...
        rejected_layout(
            LayoutSupport::IncompatibleBtrfs,
            "/home is on /dev/loop-other, expected /dev/loop-waypoint-test",
        ),
        rejected_layout(
    ...
    let missing_initramfs_store = root.join("@snapshots/anduinos-waypoint-missing-initramfs");
