"""Nautilus 4 menu provider for AnduinOS Waypoint Personal Files history.
...
Waypoint application over the user's session bus.
"""
...
BUS_NAME = "org.anduinos.Waypoint"
OBJECT_PATH = "/org/anduinos/Waypoint"
...
def _local_home_uri(file_info: Nautilus.FileInfo, require_directory: bool) -> str | None:
    ...
    # Refuse symlinks, including symlinked parent components. Waypoint performs
...
def _activate_waypoint(mode: str, uri: str) -> None:
    try:
        ...
        logging.getLogger(__name__).warning("Could not activate Waypoint: %s", error)


class WaypointFileHistoryProvider(GObject.GObject, Nautilus.MenuProvider):
    ...
    def get_file_items(self, files):
        ...
        item = Nautilus.MenuItem(
            name="AnduinOSWaypoint::ViewFileHistory",
            label=_("View File History…"),
            tip=_("Browse earlier Personal Files versions of this item"),
        )
        item.connect("activate", lambda _item: _activate_waypoint("selection", uri))
        return (item,)
    ...
    def get_background_items(self, current_folder):
        ...
        if uri is None:
            return ()
        item = Nautilus.MenuItem(
            name="AnduinOSWaypoint::BrowseFolderHistory",
            label=_("Browse This Folder’s History…"),
            tip=_("Find files that existed earlier in this Personal Files folder"),
        )
        item.connect("activate", lambda _item: _activate_waypoint("folder", uri))
        return (item,)
