...
BINARY="$ROOT/src/target/debug/anduinos-waypoint-gtk"
...
cargo build --manifest-path "$ROOT/src/Cargo.toml" --package anduinos-waypoint-gtk --offline

runtime_dir="$(mktemp -d -t anduinos-waypoint-gui-smoke.XXXXXX)"
broadway_pid=""
cleanup() {
...
ANDUINOS_WAYPOINT_UI_SMOKE_TEST=1 \
timeout 15s "$BINARY"
