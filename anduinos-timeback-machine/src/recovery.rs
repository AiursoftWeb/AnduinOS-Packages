...
use crate::transaction::{
    RollbackId, RollbackPhase, RollbackTransaction, TransactionError, TransactionStore,
    MAX_APPLY_ATTEMPTS,
};
...
impl Default for RecoveryEngine<SystemRecoveryFilesystem> {
    fn default() -> Self {
        Self::new("/run/anduinos-timeback/top", SystemRecoveryFilesystem)
    }
}
...
impl<F: RecoveryFilesystem> RecoveryEngine<F> {
    ...
    fn snapshot_root(&self) -> PathBuf {
        self.top_level.join("@snapshots/anduinos")
    }
...
mod tests {
    ...
    use crate::{transaction::RollbackTransaction, DEPLOYMENT_SCHEMA_VERSION};
    ...
    impl Environment {
        fn new() -> Self {
            let root =
                std::env::temp_dir().join(format!("timeback-recovery-{}", uuid::Uuid::new_v4()));
            fs::create_dir_all(root.join("@root")).unwrap();
            fs::write(root.join("@root/origin"), "current").unwrap();
            let snapshot_root = root.join("@snapshots/anduinos");
        ...
        fn phase(&self) -> RollbackPhase {
            TransactionStore::new(self.root.join("@snapshots/anduinos"))
                .load_pending()
                .unwrap()
                .unwrap()
                .phase
