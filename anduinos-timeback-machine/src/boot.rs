...
const GRUB_REBOOT: &str = "/usr/sbin/grub-reboot";
const GRUB_ENVIRONMENT: &str = "/boot/grub/grubenv";
...
impl Default for BootIntegration<SystemBootToolRunner> {
    fn default() -> Self {
        Self::new(SNAPSHOT_ROOT, SystemBootToolRunner)
    }
}
...
impl<R: BootToolRunner> BootIntegration<R> {
    ...
    pub fn verify_external_environment_block(&self) -> Result<String, BootError> {
        let output = self.runner.output(
            ...
            &[OsStr::new(GRUB_ENVIRONMENT), OsStr::new("list")],
        )?;
        let value = output
            .lines()
        ...
        let Some(value) = value else {
            return Err(BootError::new(
                ...
                "GRUB has no external Btrfs environment block",
            ));
        };
        if value.is_empty()
            || value.len() > 512
            ...
            })
        {
            return Err(BootError::new(
                BootErrorCode::UnsafeOutput,
                "GRUB returned an unsafe external environment block location",
            ));
        }
        Ok(value.to_string())
    }
    ...
    pub fn recovery_menu_entry(&self) -> Result<Option<String>, BootError> {
        ...
        Ok(Some(format!(
            "menuentry 'AnduinOS Timeback recovery' --class anduinos --class gnu-linux --id '{entry_id}' {{\n\
             \tinsmod btrfs\n\
             \tsearch --no-floppy --fs-uuid --set=root {fs_uuid}\n\
             \techo 'Starting AnduinOS system recovery…'\n\
             \tlinux {kernel_path} root=UUID={fs_uuid} ro rootflags=subvol=@root anduinos.timeback={id}\n\
    ...
    pub fn arm_pending_once(&self) -> Result<String, BootError> {
        ...
        self.runner.output(
            Path::new(GRUB_REBOOT),
            &[OsStr::new(&transaction.grub_entry_id)],
        )?;
...
mod tests {
    ...
    impl BootToolRunner for FakeTools {
        fn output(&self, program: &Path, arguments: &[&OsStr]) -> Result<String, BootError> {
            ...
            if program == Path::new(GRUB_REBOOT) {
    ...
    impl Environment {
        fn new() -> Self {
            let root = std::env::temp_dir().join(format!("timeback-boot-{}", uuid::Uuid::new_v4()));
    ...
    #[test]
    fn external_environment_block_must_be_present_and_safe() {
        let root = std::env::temp_dir();
        let supported = BootIntegration::new(
            &root,
            FakeTools {
                env: "env_block=(hd0,gpt2)256+2\n".into(),
                unsafe_path: false,
    ...
    fn emits_transaction_bound_grub_entry() {
        ...
        assert!(entry.contains(&format!("anduinos.timeback={}", environment.transaction.id)));
    ...
    fn arms_only_the_transaction_bound_grub_entry() {
        ...
        assert!(tools.calls.lock().unwrap().iter().any(|call| {
            call == &format!("{GRUB_REBOOT} {}", environment.transaction.grub_entry_id)
        }));
    }
}
