...
pub struct SnapshotsManagerConfig {
    ...
    /// Path to APT snapshot policy (default: /etc/anduinos-btrfs-snapshots-manager/apt-snapshots.toml)
...
impl Default for SnapshotsManagerConfig {
    fn default() -> Self {
        Self {
            snapshot_dir: PathBuf::from("/.snapshots/anduinos-btrfs-snapshots-manager/deployments"),
            automation_config: PathBuf::from(
                "/etc/anduinos-btrfs-snapshots-manager/automation.toml",
            ),
            apt_snapshot_policy: PathBuf::from(
                "/etc/anduinos-btrfs-snapshots-manager/apt-snapshots.toml",
            ),
...
impl SnapshotsManagerConfig {
    ...
    /// - ANDUINOS_BTRFS_SNAPSHOTS_MANAGER_SNAPSHOT_DIR: Override snapshot directory
    /// - ANDUINOS_BTRFS_SNAPSHOTS_MANAGER_METADATA_FILE: Override metadata file path
    /// - ANDUINOS_BTRFS_SNAPSHOTS_MANAGER_SCHEDULER_CONFIG: Override scheduler config path (deprecated)
    /// - ANDUINOS_BTRFS_SNAPSHOTS_MANAGER_SCHEDULES_CONFIG: Override schedules TOML config path
    /// - ANDUINOS_BTRFS_SNAPSHOTS_MANAGER_APT_POLICY: Override APT snapshot policy path
    /// - ANDUINOS_BTRFS_SNAPSHOTS_MANAGER_SCHEDULER_UNIT: Override the systemd scheduler unit
    /// - ANDUINOS_BTRFS_SNAPSHOTS_MANAGER_MIN_FREE_SPACE_GB: Override minimum free space (in GB)
    pub fn new() -> Self {
        let mut config = Self::default();

        // Override from environment variables
        if let Ok(dir) = std::env::var("ANDUINOS_BTRFS_SNAPSHOTS_MANAGER_SNAPSHOT_DIR") {
            config.snapshot_dir = PathBuf::from(dir);
        }

        if let Ok(conf) = std::env::var("ANDUINOS_BTRFS_SNAPSHOTS_MANAGER_AUTOMATION_CONFIG") {
            config.automation_config = PathBuf::from(conf);
        }

        if let Ok(conf) = std::env::var("ANDUINOS_BTRFS_SNAPSHOTS_MANAGER_APT_POLICY") {
            config.apt_snapshot_policy = PathBuf::from(conf);
        }

        if let Ok(space_gb) = std::env::var("ANDUINOS_BTRFS_SNAPSHOTS_MANAGER_MIN_FREE_SPACE_GB")
