...
NEW_TRANSLATIONS = {
    "A limited balance only relocates data and metadata block groups that are at most 50% full.": "受限平衡仅移动使用率不超过 50% 的数据和元数据块组。",
    ...
    "Last run completed without errors": "上次运行完成，未发现错误",
    "Last run found uncorrectable errors": "上次运行发现无法修复的错误",
    "Last run repaired errors": "上次运行已修复错误",
    "Live information reported by the mounted Btrfs file system.": "由当前挂载的 Btrfs 文件系统报告的实时信息。",
    "Maintenance": "维护",
    "No completed run recorded": "没有已完成的运行记录",
    "Not running": "未运行",
    ...
    "Scrub reads allocated data and metadata, verifies checksums, and repairs damage when another valid copy exists.": "Scrub 会读取已分配的数据和元数据、验证校验和，并在存在有效副本时修复损坏。",
    ...
    "Start a limited balance?": "开始受限平衡？",
