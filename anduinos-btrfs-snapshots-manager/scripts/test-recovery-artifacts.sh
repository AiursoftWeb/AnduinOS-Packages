...
grep -Fq 'runtime_root=/run/anduinos-btrfs-snapshots-manager' "$DRACUT_HOOK"
grep -Fq 'cp "$confirmation_engine" "$runtime_confirmation"' "$DRACUT_HOOK"
grep -Fq 'chmod 0700 "$runtime_confirmation"' "$DRACUT_HOOK"
grep -Fq -- '--stage-confirmation-artifact "$runtime_confirmation"' "$DRACUT_HOOK"
