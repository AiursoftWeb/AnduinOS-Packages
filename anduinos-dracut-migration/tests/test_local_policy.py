#!/usr/bin/env python3
"""Package-local boot policy checks."""
from pathlib import Path
import re
import unittest
import xml.etree.ElementTree as ET

ROOT = Path(__file__).resolve().parents[1]

class LocalPolicyTests(unittest.TestCase):
    def test_own_package_contract(self):
        project = ET.parse(next(ROOT.glob("*.aosproj"))).getroot()
        dependencies = {item.get("Include") for item in project.iter("Dependency")}
        service = (ROOT / "assets/anduinos-dracut-migration.service").read_text()
        self.assertNotIn("ConditionPathExists=/usr/sbin/update-initramfs", service)
        self.assertIn("ConditionPathExists=!/var/lib/anduinos-dracut-migration/complete", service)
        self.assertNotIn("update-initramfs", (ROOT / "assets/anduinos-dracut-migrate").read_text())


if __name__ == "__main__":
    unittest.main()
