...
echo "Building anduinos-appearance for architecture: $ARCH"
...
if [ "$ARCH" = "arm64" ]; then
    ...
    cp target/aarch64-unknown-linux-gnu/release/anduinos-appearance obj/anduinos-appearance
else
    need_cmd cargo

    cargo test
    cargo build --release
    cp target/release/anduinos-appearance obj/anduinos-appearance
fi
